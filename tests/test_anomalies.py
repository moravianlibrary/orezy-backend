...
class TestAnomalies:
    ...
    def test_flag_missing_pages_no_flags_when_many_single_pages(self):
        ...
        scans = [
            ...
            self.create_scan("scan3.jpg", [self.sample_page_1, self.sample_page_2]),  # double
        ]
    ...
    def test_flag_missing_pages_flags_when_few_single_pages(self):
        ...
        scans = [
            self.create_scan("scan1.jpg", [
                Page(_id=ObjectId(), xc=0.3, yc=0.4, width=0.2, height=0.3, confidence=0.8, flags=[])
            ]),  # single
            self.create_scan("scan2.jpg", [
                Page(_id=ObjectId(), xc=0.3, yc=0.4, width=0.2, height=0.3, confidence=0.8, flags=[]),
                Page(_id=ObjectId(), xc=0.7, yc=0.4, width=0.2, height=0.3, confidence=0.9, flags=[])
            ]),  # double
            self.create_scan("scan3.jpg", [
                Page(_id=ObjectId(), xc=0.3, yc=0.4, width=0.2, height=0.3, confidence=0.8, flags=[]),
                Page(_id=ObjectId(), xc=0.7, yc=0.4, width=0.2, height=0.3, confidence=0.9, flags=[])
            ]),  # double
            self.create_scan("scan4.jpg", [
                Page(_id=ObjectId(), xc=0.3, yc=0.4, width=0.2, height=0.3, confidence=0.8, flags=[]),
                Page(_id=ObjectId(), xc=0.7, yc=0.4, width=0.2, height=0.3, confidence=0.9, flags=[])
            ]),  # double
            self.create_scan("scan5.jpg", [
                Page(_id=ObjectId(), xc=0.3, yc=0.4, width=0.2, height=0.3, confidence=0.8, flags=[]),
                Page(_id=ObjectId(), xc=0.7, yc=0.4, width=0.2, height=0.3, confidence=0.9, flags=[])
            ]),  # double
        ]
    ...
    def test_flag_low_confidence_default_threshold(self):
        ...
        scans = [
            ...
            self.create_scan("scan2.jpg", [self.low_confidence_page]),  # confidence 0.5 < 0.7
        ]
    ...
    def test_flag_dimensions_anomalies_normal_pages(self):
        ...
        scans = [self.create_scan(f"scan{i}.jpg", [page]) for i, page in enumerate(normal_pages)]
    ...
    def test_flag_prediction_overlaps_functional(self):
        ...
        overlapping_pages = [
            Page(_id=ObjectId(), xc=0.5, yc=0.5, width=0.3, height=0.3, confidence=0.8, flags=[]),
            Page(_id=ObjectId(), xc=0.5, yc=0.5, width=0.2, height=0.2, confidence=0.9, flags=[])  # Same center, smaller
        ]
    ...
    def test_flag_prediction_overlaps_no_overlap(self, mock_cxywh_to_xyxy, mock_bbox_intersection):
        """Test that non-overlapping pages don't get flagged."""
        # Mock the utility functions
        mock_cxywh_to_xyxy.side_effect = lambda xc, yc, w, h: (xc-w//2, yc-h//2, xc+w//2, yc+h//2)
    ...
    def test_multiple_anomaly_functions_integration(self):
        ...
        scans = [
            self.create_scan("scan1.jpg", [low_conf_page]),  # Single page + low confidence
            self.create_scan("scan2.jpg", [
                Page(_id=ObjectId(), xc=0.3, yc=0.4, width=0.2, height=0.3, confidence=0.8, flags=[]),
                Page(_id=ObjectId(), xc=0.7, yc=0.4, width=0.2, height=0.3, confidence=0.9, flags=[])
            ]),  # Double page
        ...
        assert len(single_scan.predicted_pages[0].flags) >= 2  # Should have multiple flags
        assert Anomaly.page_count_mismatch in single_scan.predicted_pages[0].flags
