...
class TestApiUtils:
    ...
    def create_scan(
        self,
        filename: str,
        predicted_pages: list[Page],
        user_edited_pages: list[Page] = None,
    ) -> Scan:
    ...
    def test_format_predicted_basic_functionality(self):
        ...
        assert result[0]["_id"] == str(scan.id)
        assert (
            "edited" not in result[0]
        )  # format_predicted doesn't include edited field
        assert "flag1" in result[0]["flags"]
        assert "flag2" in result[0]["flags"]
    ...
    def test_format_predicted_ignores_user_edited_pages(self):
        ...
        assert len(result[0]["pages"]) == 1
        assert (
            result[0]["pages"][0]["type"] == "left"
        )  # from page1, not "single" from edited_page
        assert (
            result[0]["pages"][0]["xc"] == 0.25
        )  # from page1, not 0.3 from edited_page
    ...
    def test_page_object_to_dict_empty_list(
        self, mock_jsonable_encoder, mock_cxywh_norm_to_xyxy
    ):
    ...
    def test_integration_format_functions_together(self):
        ...
        scan = self.create_scan(
            "integration_test.jpg", [self.page1, self.page2], [self.edited_page]
        )
